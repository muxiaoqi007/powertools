...
public sealed class PowerQueryExportService
{
    ...
    private static IReadOnlyDictionary<string, EntityDefinition> BuildDefinitions()
    {
        var definitions = new[]
        {
            ...
            Def("columns", "模型字段", C("tableName", "columnName", "dataType", "isHidden", "isCalculated", "expression", "description", "sortByColumn", "isKey", "isUnique"), s => s.Tables.SelectMany(t => t.Columns.Select(c => Row(("tableName", t.Name), ("columnName", c.Name), ("dataType", c.DataType), ("isHidden", c.IsHidden), ("isCalculated", c.IsCalculated), ("expression", c.Expression), ("description", c.Description), ("sortByColumn", c.SortByColumn), ("isKey", c.IsKey), ("isUnique", c.IsUnique))))),
            ...
            Def("report-quality", "报表质量汇总", C("reportFilterCount", "pageFilterCount", "visualFilterCount", "tooltipPageCount", "drillthroughPageCount", "dataVisualCount", "explicitTitleCount", "altTextCount", "filteredVisualCount"), s => ReportQualityRows(s)),
            Def("pages", "报表页面", C("pageName", "displayName", "width", "height", "isHidden", "displayOption", "visualCount", "filterCount", "drillthroughFilterCount", "isTooltip"), s => s.Pages.Select(p => Row(("pageName", p.Name), ("displayName", p.DisplayName), ("width", p.Width), ("height", p.Height), ("isHidden", p.IsHidden), ("displayOption", p.DisplayOption), ("visualCount", p.Visuals.Count), ("filterCount", p.FilterCount), ("drillthroughFilterCount", p.DrillthroughFilterCount), ("isTooltip", p.IsTooltip)))),
            Def("visuals", "页面视觉对象", C("pageName", "pageDisplayName", "visualName", "title", "visualType", "x", "y", "width", "height", "z", "tabOrder", "isHidden", "fieldCount", "filterCount", "hasExplicitTitle", "hasAltText", "hasTooltip", "drillFilterOtherVisuals", "isGroup", "sourceFile"), s => s.Pages.SelectMany(p => p.Visuals.Select(v => Row(("pageName", p.Name), ("pageDisplayName", p.DisplayName), ("visualName", v.Name), ("title", v.Title), ("visualType", v.Type), ("x", v.X), ("y", v.Y), ("width", v.Width), ("height", v.Height), ("z", v.Z), ("tabOrder", v.TabOrder), ("isHidden", v.IsHidden), ("fieldCount", v.Fields.Count), ("filterCount", v.FilterCount), ("hasExplicitTitle", v.HasExplicitTitle), ("hasAltText", v.HasAltText), ("hasTooltip", v.HasTooltip), ("drillFilterOtherVisuals", v.DrillFilterOtherVisuals), ("isGroup", v.IsGroup), ("sourceFile", v.SourceFile))))),
            ...
            Def("issues", "质量检查问题", C("issueId", "severity", "category", "title", "detail", "objectName", "pageName", "targetId"), s => s.Issues.Select(i => Row(("issueId", i.Id), ("severity", i.Severity), ("category", i.Category), ("title", i.Title), ("detail", i.Detail), ("objectName", i.ObjectName), ("pageName", i.PageName), ("targetId", i.TargetId)))),
            Def("storage-metrics", "VertiPaq 字段存储指标", C("tableName", "columnName", "rowCount", "cardinality", "dataSizeBytes", "dictionarySizeBytes", "hierarchySizeBytes", "totalSizeBytes", "segmentCount"), s => s.StorageMetrics.Select(x => Row(("tableName", x.TableName), ("columnName", x.ColumnName), ("rowCount", x.RowCount), ("cardinality", x.Cardinality), ("dataSizeBytes", x.DataSizeBytes), ("dictionarySizeBytes", x.DictionarySizeBytes), ("hierarchySizeBytes", x.HierarchySizeBytes), ("totalSizeBytes", x.TotalSizeBytes), ("segmentCount", x.SegmentCount)))),
            Def("removal-candidates", "字段与度量删除候选", C("objectType", "tableName", "objectName", "status", "confidence", "riskScore", "estimatedSavingsBytes", "reasons", "evidence"), s => s.RemovalCandidates.Select(x => Row(("objectType", x.ObjectType), ("tableName", x.TableName), ("objectName", x.ObjectName), ("status", x.Status), ("confidence", x.Confidence), ("riskScore", x.RiskScore), ("estimatedSavingsBytes", x.EstimatedSavingsBytes), ("reasons", string.Join(" | ", x.Reasons)), ("evidence", string.Join(" | ", x.Evidence))))),
    ...
    private static IEnumerable<IReadOnlyDictionary<string, object?>> SummaryRows(ProjectSnapshot s)
    {
        ...
        yield return Row(("metric", "reportFilterCount"), ("value", s.ReportFilterCount));
        yield return Row(("metric", "pageFilterCount"), ("value", s.Pages.Sum(p => p.FilterCount)));
        yield return Row(("metric", "visualFilterCount"), ("value", s.Pages.Sum(p => p.Visuals.Sum(v => v.FilterCount))));
    ...
    private static IEnumerable<IReadOnlyDictionary<string, object?>> ReportQualityRows(ProjectSnapshot s)
    {
        var visuals = s.Pages.SelectMany(page => page.Visuals).ToList();
        yield return Row(
            ("reportFilterCount", s.ReportFilterCount),
            ("pageFilterCount", s.Pages.Sum(page => page.FilterCount)),
            ("visualFilterCount", visuals.Sum(visual => visual.FilterCount)),
            ("tooltipPageCount", s.Pages.Count(page => page.IsTooltip)),
            ("drillthroughPageCount", s.Pages.Count(page => page.DrillthroughFilterCount > 0)),
            ("dataVisualCount", visuals.Count(visual => !visual.IsHidden && !visual.IsDecorative && !visual.IsGroup)),
            ("explicitTitleCount", visuals.Count(visual => !visual.IsHidden && !visual.IsDecorative && !visual.IsGroup && visual.HasExplicitTitle)),
            ("altTextCount", visuals.Count(visual => !visual.IsHidden && !visual.IsDecorative && !visual.IsGroup && visual.HasAltText)),
            ("filteredVisualCount", visuals.Count(visual => visual.FilterCount > 0)));
    }
