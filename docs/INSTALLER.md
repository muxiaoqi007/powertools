...
PowerTools 0.9.1 提供标准 x64 Windows 安装程序：

```text
PowerTools-Setup-0.9.1-win-x64.exe
```
...
.\scripts\build-installer.ps1 -Version 0.9.1
```

输出位置：

```text
artifacts\installer\PowerTools-Setup-0.9.1-win-x64.exe
```

构建默认执行自包含发布。若只需要体积更小、依赖目标电脑 .NET 8 的安装包，可使用：

```powershell
.\scripts\build-installer.ps1 -Version 0.9.1 -FrameworkDependent
```
...
```powershell
PowerTools-Setup-0.9.1-win-x64.exe /VERYSILENT /SUPPRESSMSGBOXES /NORESTART /SP-
```
